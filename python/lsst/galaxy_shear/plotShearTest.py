...
#   filter                plot only runs with the specified filter
    ...
    #  But if it is a test involving multiple runs, call runPlot multiple times, then display a graph of all the fits.
        ...
        files = glob.glob("subfield_anal_%s*.fits"%(args.test))
        m1s = []
        m2s = []
        b1s = []
        b2s = []
        ...
        for file in files:
            ...
            b1errs.append(b1err)
            b2errs.append(b2err)
            start = len("subfield_anal_%s"%(args.test))
            testnumbers.append(float(file[start:file.find('.fits')]))
        if args.display:
            import matplotlib.pyplot as plot
            ...
            yrange = 1.5 * max((m1s.max() - m1s.min() + m1errs.max(), m2s.max() - m2s.min() + m2errs.max())) 
            # plot the data with error bars
            ...
            # put both graphs on the same units scale
            yrange = 1.5 * max((b1s.max() - b1s.min() + b1errs.max(), b2s.max() - b2s.min() + b2errs.max())) 
